...
- 游戏进程出现时只登记目标 PID，不创建 worker。注入启动必须同时确认 helper 完成全部
  远程 `LoadLibrary` 和游戏 Running，并在可选主动就绪后单独等待五秒；恢复游戏后不再
  枚举受保护进程模块。主程序观察 Hook 在注入前卸载、最终阶段先重装，随后才首次创建 worker，使负责
