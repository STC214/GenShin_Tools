...
- 启动确认：`restarting` phase；新版本确认后清理；确认超时或启动失败时尝试恢复上一版本。
...
以上门禁在本记录日期均通过。默认 `go vet ./...` 仍会报告 Win32 ABI callback/LPARAM 边界的 `unsafe.Pointer` 提示；这些位置已由 `scripts/test.ps1` 使用 `-unsafeptr=false` 隔离，并保留在全量审计报告中供人工复核。
...
真实 Windows 多显示器/DPI 长时间运行、真实游戏输入 soak、休眠唤醒和干净机器首次安装属于 S13，按当前工作要求暂不执行。
