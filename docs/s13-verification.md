...
2026-07-26 当轮机器可读记录曾输出到 `artifacts/s13/automated-verification.json`；历史临时验证目录已在 1.5.6 制品整理时清理，当前发布目录只保留最新版 ZIP 与 sidecar。
...
## 1.4.9 内置 worker 完整退役与审查修复

- 鼠标连点在启动/注入门禁关闭期间保持 `Enabled + Armed`，安全监视器不再提前进入
  Running，也不会因首个 `SendInput` 被门禁拒绝而把会话误判为目标丢失；门禁放行后
  才允许在已核验的前台游戏窗口开始输出。
- 新配置默认使用左键连点模式。退役的连发键列表和键盘模式开关键不再参与鼠标开关、
  全局停止或截图热键冲突，隐藏旧字段无法继续占用用户可见快捷键。
- AHK 启动失败会显示在输入增强页，重试采用 1、2、4、8、16、30 秒的有界退避；
  启动成功、关闭“随游戏启动/关闭”或游戏退出时清除错误。
- 正式构建、便携包和更新契约不再包含 `GenshinTools-input.exe`；升级后首次启动会
  幂等清理安装根目录中的旧文件。旧 worker 源码仅保留为历史审计材料。
- 便携打包和独立发布审计现在要求 `build-info.json` 携带 12～40 位十六进制干净
  Git 提交号，拒绝 `-dirty`、`uncommitted`、`unknown` 或缺失提交的候选包。
- 2026-07-29 全项目普通/race、格式、依赖和 vet 策略门禁通过；dirty 工作树构建只
  生成主程序、注入 helper 和更新 helper，确认没有 x86 input worker。PE、版本、
  图标、权限和真实注入 helper 协议检查通过后，构建审计在 `build-info.json` 处按
  设计拒绝 `793e07aeeca0-dirty`，便携打包也拒绝生成 ZIP。必须先形成干净 Git
  提交，再生成可发布的 1.4.9 候选包。

## 1.5.0 AHK 失败关闭与任务拒绝修复

- AHK 启动产生新 PID、但后续审计/Job 绑定失败且无法确认回滚时，启动任务现在保留
  组合错误并停止自动重试。只有任务上下文确实因生命周期切换被取消、选项仍开启、
  游戏仍存在且程序不在退出时，defer 才会投递一次协调消息；致命回滚失败不能再借
  defer 绕过“禁止重试”约束并累积无法托管的 AHK 进程。
- 任务管理器进入关闭状态后会以任务 ID 0 拒绝新工作。AHK 调度现在显式释放
  `ahkStarting` CAS 门禁，避免任务体和 defer 未执行时永久卡在“正在启动”。
- 新增真值表回归覆盖快速 off/on、致命回滚、选项关闭、游戏退出和程序关闭五种协调
  条件，并覆盖任务管理器拒绝 AHK 工作时的状态释放。

## 1.5.1 AHK 致命重试门禁与发布失败清理

- AHK 致命回滚失败新增独立原子阻断位，并纳入调度函数最外层前置条件。错误文本仍可
  通过 `messageInput` 请求 UI 刷新，但同一消息随后调用调度器时也无法绕过阻断位；
  只有用户切换 AHK 生命周期选项、游戏生命周期结束或后续成功托管时才清除。
- 便携包 ZIP 写成后、内部重新解包审计或 `.sha256` 原子落盘失败时，会删除新候选和
  旧的普通校验和文件，不能留下缺少审计闭环的 ZIP。目录/reparse 等非普通校验和
  目标不会被清理逻辑误删。
- ZIP 与校验和的最终替换改用项目统一的 `MoveFileExW(REPLACE_EXISTING |
  WRITE_THROUGH)` 有界重试封装，不再先删除目标再改名，避免崩溃窗口丢失原候选。
- 新增回归覆盖错误 UI 消息触发调度时的 fatal block、任务拒绝门禁，以及校验和替换
  失败后候选包清理；既有重复打包测试同时覆盖原子覆盖已存在 ZIP/校验和。

## 1.5.2 便携部署严格镜像

- 审查发现重复使用 `Expand-Archive -Force` 只会覆盖同名文件，不会删除新版本已经
  退休的文件；实际部署目录因此残留旧 AutoHotkey v1.1 许可证与对应源码，和当前
  AHK_F 内嵌的 v1.0.48.05 运行时不一致。
- 新增 `tools/portable-deploy` 与 `scripts/deploy-portable.ps1`。部署前重新验证 ZIP
  整体哈希、`release.json` 契约及每个条目的大小/哈希；目标目录只保留 `data/`，
  其他产品文件通过同卷备份后严格镜像当前包。
- 安装任一条目失败时会按逆序撤回新文件并恢复旧文件，避免覆盖到一半留下混合版本；
  便携包若尝试携带 `data/` 会在修改目标前直接拒绝。
- 新增回归覆盖用户配置保留、旧顶层文件与旧许可证目录删除、当前文件安装，以及
  包内 `data/` 拒绝且用户配置不变。

## 1.5.3 便携部署崩溃恢复与并发互斥

- 1.5.2 的部署回滚清单只存在于进程内存：普通文件错误可以恢复，但部署进程被强杀、
  系统重启或断电会丢失清单，目标可能停在旧文件已备份或新文件只安装一部分的状态。
- 部署器现在在目标同卷父目录写入独占事务日志，并在备份、安装、恢复、已回滚和已
  提交阶段之间使用原子 `MoveFileExW(REPLACE_EXISTING | WRITE_THROUGH)` 更新。
  下一次部署先恢复未完成事务：安装未提交则删除新文件并恢复旧文件，提交完成则只
  清理旧备份；`data/` 始终留在原位。
- 回滚自身若在恢复多个旧条目之间再次失败，会先持久化 `restoring` 阶段；下次运行
  只恢复剩余备份，不会把已经恢复的旧文件当成新文件删除。
- 以规范化目标路径的 SHA-256 派生全局命名互斥体，拒绝同一目标在并发进程或不同
  Windows 会话中同时部署；互斥体由内核在进程异常退出时自动释放。
- 新增回归覆盖并发所有权、安装中断、恢复再次中断后的幂等续作、已提交清理、正常
  错误回滚、用户数据保持和退休文件删除。

## 1.5.4 已提交部署复核与候选包保全

- 部署事务写入 `committed` 后不再无条件删除旧备份。新版日志记录当前便携包全部
  顶层产品条目；清理备份前重新验证目标是真实目录、所有声明条目齐全、没有额外
  产品条目，且可选 `data/` 仍为真实目录。目标被删除、隔离或篡改时保留日志和旧
  备份并明确失败。
- 事务日志架构升级为 2，同时保留对 1.5.3 架构 1 崩溃日志的恢复兼容；旧日志在
  committed 清理前至少要求真实目标目录中仍存在产品文件。日志中的源、备份和条目
  名称增加绝对路径、同父目录、唯一性和路径分隔符校验。
- 便携打包改为先在隐藏候选名下完成 ZIP 写入、整体哈希和重新解包审计，之后才发布
  最终文件；审计失败不会覆盖或删除上一份有效候选。
- 同一版本已有内容不同的 ZIP 时拒绝覆盖，确保版本不可变；内容相同的确定性重跑只
  修复校验和。首次发布先原子落盘校验和、最后发布 ZIP，进程崩溃最多留下不可用的
  孤立校验和，不会暴露缺少匹配校验和的未审计 ZIP。
- 新增回归覆盖 committed 目标缺失/多余条目时保留旧备份、正常 committed 清理、
  相同候选的校验和失败保全，以及不同同版本内容拒绝覆盖。

## 1.5.5 注入失败现场诊断

- 生产日志复核发现注入启动失败只更新界面状态，后台 helper、审计或兼容工具停止错误没有持久记录；helper staging 又会在请求结束后清理，导致事后缺少精确错误文本。
- 启动前后新增结构化日志，记录有效任务的失败种类、任务 ID 和完整错误；同步模块预检和输入 Hook 准备错误也单独落盘。
- 初版兼容处理在 `TerminateProcess` 返回错误后检查进程是否已经自然退出，避免把已达到停止状态的进程误报为启动失败。
- 该版本用于定位与部署验证，后续审查发现进程身份辅助函数仍按 PID 重开，且构建标识曾由环境覆盖；两项均在 1.5.6 收口。因此当前发布目录不保留 1.5.5 候选包。

## 1.5.6 单句柄进程身份与可追踪候选包

- 捕获的 AHK/QuickInput 只打开一次目标进程，随后从同一 HANDLE 读取创建时间、完整映像路径和 signaled 状态，关闭 PID 重开与复用窗口。
- 终止前先检查退出状态；`TerminateProcess` 报错后再有界等待 100ms。原 HANDLE 已 signaled 时确认自然退出，仍运行时返回原终止错误；成功终止后最多等待 2 秒确认退出。
- 新增确定性测试覆盖已退出/仍运行状态、`ERROR_ACCESS_DENIED` 后自然退出、拒绝终止且仍运行，以及注入启动失败 JSON Lines 日志字段。
- 修复完整提交为 `9c4588a8847a6a850992819aceded3659ee7576c`。构建未设置 `BUILD_COMMIT` 覆盖，`build-info.json` 自动记录 `9c4588a8847a`，发布身份与源码一致。
- `scripts/test.ps1`、PE/版本/清单审计、helper 请求/结果回写、便携布局和 ZIP 重开全部通过。
- 文件：`artifacts/release/GenshinTools-1.5.6-windows-amd64-portable.zip`
- 大小：9,676,186 bytes
- SHA-256：`b7b24cb7a0de39ea4b02972c0539fc3d83bddc6ab851b2cdfea8e5aeed0529fd`
- 条目数：18；不含 `data/`、Debug EXE、历史候选或临时验证文件。
