...
更新时间：2026-07-19
状态：短时自动发布矩阵已通过；真实桌面/游戏、许可证和上游 disposition 人工门禁未关闭。
...
- 大小：7,355,038 bytes
- SHA-256：`e95e02173af73d2d9887286479a3bf4c652ff069ac004ee4dc8dc3c3c1ec9227`
- ZIP 条目：12 个，仅包含 `release.json`、三个 Release EXE、`build-info.json`、第三方通知、许可证政策和 `LICENSES/` 文本。
- 明确不含：Debug EXE、`data/`、日志、缓存、staging、测试夹具和源码。

候选 ZIP 使用固定条目顺序和时间戳；本轮构建时间固定为 `2026-07-18T17:29:19Z`（Git HEAD 提交时间）。清洁重建四个 EXE 后再次打包得到相同大小和 SHA-256，确认相同源码、提交身份和工具链可复现。源码、提交或显式构建时间改变会按设计改变 `build-info.json` 和包哈希。
...
5. 选择项目级许可证。
6. 完成 scope-v2 上游报告的人工 disposition。
