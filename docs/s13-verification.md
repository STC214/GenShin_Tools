...
候选 ZIP 使用固定条目顺序和时间戳；最终构建时间、源提交和哈希在本次文档与第三方通知提交后重新记录。清洁重建四个 EXE 后再次打包得到相同大小和 SHA-256，可确认相同源码、提交身份和工具链可复现。源码、提交或显式构建时间改变会按设计改变 `build-info.json` 和包哈希。
