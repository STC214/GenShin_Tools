...
更新时间：2026-07-22
状态：全量自动发布矩阵与上游 disposition 已通过；真实桌面/游戏和项目许可证人工门禁未关闭。
...
- 大小：7,420,031 bytes
- SHA-256：`250b8f4f7ab90ca631ea3f5f6f8887dfadfc3a912e34ce59d80037104dfcd364`
- ZIP 条目：13 个，仅包含 `release.json`、三个 Release EXE、`build-info.json`、第三方通知、许可证政策和 `LICENSES/` 文本。
- 明确不含：Debug EXE、`data/`、日志、缓存、staging、测试夹具和源码。

候选 ZIP 使用固定条目顺序和时间戳；本轮构建时间固定为 `2026-07-22T14:36:42Z`（Git HEAD `eccb5fb978698bde88e9992b0f26cf4edd9fed0d` 的提交时间）。清洁重建四个 EXE 后再次打包得到相同大小和 SHA-256，可确认相同源码、提交身份和工具链可复现。源码、提交或显式构建时间改变会按设计改变 `build-info.json` 和包哈希。
