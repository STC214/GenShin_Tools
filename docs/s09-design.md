...
- 可选 `readyEvent` 必须严格为 `Local\GenshinTools.PluginReady.<id>.{pid}`。选择该协议的模块应创建手动复位事件，并仅在异步初始化和 Hook 安装全部结束后置位；PID 占位符由启动器按本次游戏生命周期展开。未声明该字段的旧模块以 helper 的全部远程 `LoadLibrary` 成功返回作为同步注入完成点；两种路径之后都另行等待五秒才加载输入 Hook。
...
- 注入前捕获的兼容输入工具必须通过同一个已打开进程句柄核验创建时间、完整映像路径和退出状态，禁止按 PID 重开后再终止。自然退出与终止调用并发时，以该句柄的 signaled 状态作为最终结果。
- 审计、启动预检、输入准备及 helper 的异步失败除显示在 UI 外，还必须写入结构化日志；任务日志至少包含阶段种类、任务 ID 和完整错误文本。
...
5. 关闭注入时与 S05 `NativeStarter` 路径一致；Debug/GUI/helper 构建和版本元数据通过。
6. 兼容工具已退出、仍运行且拒绝终止两条分支均有确定性测试；helper/审计失败日志有落盘断言。
