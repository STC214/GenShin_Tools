...
游戏 Running 两个信号都到达，且游戏连续前台 8 秒，才按“内置 worker → 外部工具 →
便携 AHK”顺序加载。重复的 2 秒游戏扫描不会重置这段等待，游戏生命期变化、取消或失败
则关闭门禁并恢复被停止的用户工具。

最终审查修订进一步覆盖完整 Hook 链：主程序自身的 `WH_KEYBOARD_LL`/
`WH_MOUSE_LL` 观察 Hook 也在注入前卸载，最终按“主观察 Hook → 内置 worker → 外部
工具 → 便携 AHK”重装；8 秒稳定窗口同时持续枚举并核对全部注入 DLL 仍驻留于准确游戏
PID。待处理外部工具使用代际号提交，等待取消或游戏 PID 集合变化只会把准确生命周期
