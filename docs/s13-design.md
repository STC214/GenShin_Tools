...
`scripts/package-portable.ps1` 只打包 Release 主程序、两个固定 helper、`build-info.json`、第三方通知和扁平许可证目录。Debug EXE、运行数据、缓存、日志、staging 和开发文件不会进入 ZIP。

ZIP 内含严格排序的 `release.json`，每个文件记录长度和 SHA-256。S13 使用 `SOURCE_DATE_EPOCH`（若提供）或 Git HEAD 提交时间作为确定性构建时间，打包工具使用固定 ZIP 时间戳和顺序，随后调用 S12 的 `StagePackage` 重新验证整个包。输出名称使用 `portable` 表明其为免安装便携包；是否达到正式公开发布条件仍由真实桌面/游戏人工门禁决定。

可发布构建必须来自干净 Git 工作树，并由构建脚本自动取得提交号；`BUILD_COMMIT` 只用于不含 `.git` 的源码归档，不能用来覆盖普通工作树的 dirty 状态。版本化 ZIP 与 sidecar 统一位于 `artifacts/release/`，仓库根目录不保存历史候选。`scripts/clean.ps1` 只清理可再生的构建目录和缓存，不删除已经验证的发布 ZIP。
